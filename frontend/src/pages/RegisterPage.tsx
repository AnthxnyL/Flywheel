...
export default function RegisterPage() {
  ...
  // Only dealers can self-register — drivers are invited by their dealer
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--color-background)] px-4">
        <div className="w-full max-w-md bg-[var(--color-surface)] rounded-2xl shadow-sm border border-[var(--color-border)] p-8 text-center">
          ...
          <p className="text-[var(--color-text-secondary)]">Un lien de confirmation a été envoyé à <strong>{email}</strong>.</p>
          <Link to="/login" className="mt-6 inline-block text-[var(--color-primary)] font-medium hover:underline">Aller à la connexion</Link>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-background)] px-4">
      <div className="w-full max-w-md bg-[var(--color-surface)] rounded-2xl shadow-sm border border-[var(--color-border)] p-8">
        ...
        <p className="mt-6 text-center text-sm text-[var(--color-text-secondary)]">
          Déjà un compte ?{' '}
          <Link to="/login" className="text-[var(--color-primary)] font-medium hover:underline">Se connecter</Link>
        </p>
      </div>
    </div>
  )
